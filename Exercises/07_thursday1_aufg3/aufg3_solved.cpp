...
template<class ElemType, class InIt, class OutIt>
OutIt real_unique_copy(
	InIt beg,
	InIt end,
	OutIt dest
) {
	...
	while (beg != end) {
		const ElemType elem = *beg++;	// fulfill "contract"
						// for input iterator
		if (known.insert(elem).second) {
			*dest++ = elem;		// fulfill "contract"
						// for output iterator
		}
	}
	return dest;
...
int main(int argc, char* argv[]) {
		...
		.run_to_return(
    		...
    		.run_to_return(
    			[](istream&, ostream& out) {
    			// ==================================================
    			const vector<double> data = {5.6, 3.5, 7, 3.5, 4, 7};
