...
void real_unique_copy(
	...
	std::set<std::string> known;			// words already known
	while (beg != end) {
#if 1
		const std::string elem = *beg++;	// fulfill contract for
							// input iterator
		if (known.insert(elem).second)
			*dest++ = elem;			// fulfill contract for
							// output iterator
#else
		{
			using namespace std;
			typedef pair<set<string>::iterator, bool> insert_result;
			const insert_result result = known.insert(*beg++);
			// const auto result = known.insert(*beg++);
			if (result.second)
				*dest++ = *result.first;
		}
#endif
